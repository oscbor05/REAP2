...
def assess_property_complexity(property_name, revenue, onboarding_status, nps_score, 
                              hospitality_service, financial_acumen, special_assessments, 
                              solvency, investment_accounts, cash_accounts, amenities, projects):
    """ Assesses the complexity of a property based on KWPMC REAP Scoring Guide."""
    revenue_code = "L" if revenue < 750000 else "M" if revenue <= 1100000 else "H"
    
    expectation_score_operational = sum([
        0 if nps_score >= 30 else 3,
        0 if onboarding_status == "Not onboarded / More than 90 days" else 3,
        3 if hospitality_service == "Yes" else 1
    ])
    
    expectation_score_financial = sum([
        0 if financial_acumen == "Strong" else 2 if financial_acumen == "Moderate" else 3,
        0 if special_assessments == "No" else 3,
        0 if solvency == "Yes" else 3,
        0 if investment_accounts == "No" else 3,
        0 if cash_accounts <= 2 else 2 if cash_accounts <= 5 else 3
    ])
    ...
    return {
        ...
        "Expectation Level Score": expectation_score,
